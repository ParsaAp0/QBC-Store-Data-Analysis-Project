...
class XGBBinaryModel(model_interface):
        def __init__(self):
                super().__init__("XGBoost Binary Classifier", "classification")

                self.target_column = target_feature
                self._load_best_params()

                # Binary classification mapping
                self.binary_map = {'Same Day': 1, 'Not Same Day': 0}
                self.reverse_binary_map = {v: k for k, v in self.binary_map.items()}

                # Model – binary classification (objective will be set automatically)
        ...
        def predict(self, test_data: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
                ...
                # Predict numeric binary (0/1)
                preds_numeric = self.model.predict(X)

                preds_string = np.array([self.reverse_binary_map[p] for p in preds_numeric])

                y_string = np.array([self.reverse_binary_map[val] for val in y])

                return y_string, preds_string
        ...
        def tune(self, train_data: pd.DataFrame) -> dict:
                with all_params_path.open("r") as file:
                        ...
                        param_grid = {
                                ...
                                'reg_alpha': parameter_grid.get('reg_alpha', [0, 1, 5]),
                                'reg_lambda': parameter_grid.get('reg_lambda', [1, 5, 10]),
                        }
        ...
        def _encode_features(self, data: pd.DataFrame, fit: bool) -> pd.DataFrame:
                ...
                if self.categorical_columns:
                        ...
                        # Order Priority is ordinal: map to numeric
